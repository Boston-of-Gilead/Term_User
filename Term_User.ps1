...
#Open Active Directory Users and Computers and move the user to (<other OU>. #Move-ADObject -Identity $strDN 
Get-ADUser -Identity $User | Move-ADObject -TargetPath "OU=O365DisabledUsers,OU=Exchange,DC=bcc,DC=URL,DC=local" #-TargetServer "fqdn"
...
$Session = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://Exchange/powershell -Authentication Kerberos -AllowRedirection #-Credential $UserCredential 
Import-PSSession $Session -DisableNameChecking -AllowClobber
#Set-RemoteMailbox -Identity $User -Type Shared -HiddenFromAddressListsEnabled $true #$Email #Changed to below line to test
Set-Mailbox -Identity $User -Type Shared -HiddenFromAddressListsEnabled $true #$Email
...
If ($cmd | Out-String -Stream | Select-String -Pattern "SKU") {
    Write-Host "User has G1"
    $Sku = "G1"
}
Elseif ($cmd | Out-String -Stream | Select-String -Pattern "SKU") {
...
#If access to MB is needed, delegate access to that user, Full Access. User can access the MB via webmail.URL.net and selecting "Open other mailbox".
...
If ($Delegation -eq "y") {
    While ($Delegation -eq "y") {
        ...
        #Add-Mailboxpermission –Identity $Email –User $Remainer –accessrights Fullaccess, doesn't work
...
$homedirpath = "\\UNC\to\$user"
$homedocspath = "\\UNC\to\$User\My Documents"
...
if ($there -eq $True) {
    ...
    $ACL = Get-ACL "\\UNC\to\$user"#added quotes 12/7, may need to change to $homedirpath to address ctor issue
    $Owner = New-Object System.Security.Principal.NTAccount($Admin)
    $ACL.SetOwner($Owner)
    Set-Acl -Path \\UNC\to\$user -AclObject $ACL

    $src = "\\UNC\to\$User"
    $dst = "\\UNC\to\$User" #added $User 1/4/21
    $docs = "\\UNC\to\$User\My Documents"

    #take ownership of my documents
    if ($docsthere -eq $True) {
        $ACL = Get-ACL -Path "\\UNC\to\$User\My Documents" #changed to path from $docs 2/12 #$docs #1/27 added -Path
        $Owner = New-Object System.Security.Principal.NTAccount($Admin)
        $ACL.SetOwner($Owner)
        Set-Acl -Path $docs -AclObject $ACL #'\\UNC\to\$user\My Documents' -AclObject $ACL 
        }
        else{}
    ...
    if ($docsthere -eq $True) {
        $docs = "\\UNC\to\$User\My Documents"
        $acl = Get-Acl $docs
        $AccessRule = New-Object System.Security.AccessControl.FileSystemAccessRule("bcc\$admin","FullControl","Allow")
        $acl.SetAccessRule($AccessRule)
        $acl | Set-Acl $docs
        }
        else{}

    takeown /f "\\UNC\to\$User" /r

    #Move the home folder to \\UNC\to, append the name with the date of separation, e.g. jdoe01012021
    $src = "\\UNC\to\$User"
    $dst = "\\UNC\to\$User" #added $User 1/4/21
    Move-Item -Path $src -Destination $dst
    Rename-Item "\\UNC\to\$User" "\\UNC\to\$User$Date"
    ...
    $homedir = "\\UNC\to\$User$Date"  #quotes correct 1/7/21
    ...
    While ($ans -eq "y") {
        ...
        #Grant-FileShareAccess -Name $homedir -AccessRight "Read" -AccountName bcc\$Delegate
